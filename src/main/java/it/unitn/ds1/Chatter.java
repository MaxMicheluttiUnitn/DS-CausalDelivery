...
class Chatter extends AbstractActor {
  ...
  private void updateLocalClock(ChatMsg msg){
    for(int i=0;i<this.group.size();i++){
      this.vc[i]=Integer.max(this.vc[i],msg.vc[i]);
    }
  }
  ...
  */
  ...
  private boolean canDeliver(ChatMsg incoming){
    if (incoming.vc[incoming.senderId]!=this.vc[incoming.senderId]+1)
      return false;
    for(int i=0;i<this.group.size();i++){
      if((i!=incoming.senderId)&&(incoming.vc[i]>this.vc[i]))
        return false;
    }
    return true;
  }
  ...
  private ChatMsg findDeliverable() {
    Iterator<ChatMsg> iter = buffer.iterator();
    while (iter.hasNext()) {
      ChatMsg msg = iter.next();
      if (canDeliver(msg)){
        iter.remove();
        return msg;
      }
